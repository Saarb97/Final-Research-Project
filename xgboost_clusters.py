...
def objective(trial, X_train_fold, y_train_fold):
    ...
    else:                                    # binary – keep threshold adjustable if you wish
        y_pred = (y_val_proba[:, 1] >= 0.5).astype(int)
    ...
    for fold, (tr_idx, te_idx) in enumerate(kf.split(X, y)):
        ...
        if fold == 0:
          if models_dir is not None:
            ...
            joblib.dump({"model": model,          # saves the estimator
...
def main_kfold(data_files_loc, num_of_clusters, output_loc,
    ...
    if all_preds:  # protect against empty list
        ...
        # 3b.  Calibration metrics  ───────────────────────────────────────────────
        #
        #     • Brier score   – proper scoring rule: lower is better (0 = perfect)
        #     • ECE (10-bin)  – average gap between predicted prob. and empirical freq.
        ...
        if prob_cols:
            # ---------- Brier score ----------
            if proba_full.shape[1] == 2:                      # binary
                brier_glob = brier_score_loss(y_true, proba_full[:, 1])
            else:                                             # multiclass → one-vs-all then average
                y_onehot = label_binarize(y_true, classes=labels)
                brier_glob = ((proba_full - y_onehot) ** 2).sum(axis=1).mean()

            # ---------- ECE (fixed-width 10-bin) ----------
            def ece(probs, y, k=10):
                """Expected Calibration Error (vectorised for binary or multiclass)."""
                if probs.ndim == 1:           # binary → reshape to (n,1)
                    probs = probs[:, None]
                y_hot = label_binarize(y, classes=labels)
            ...
            if proba_full.shape[1] == 2:      # binary
                ece_glob = ece(proba_full[:, 1], y_true)
            else:                             # multiclass
                ece_glob = ece(proba_full, y_true)
        ...
        with open(metrics_txt, "w") as fout:
            ...
            fout.write(f"Brier Score            : {brier_glob:.4f}\n")
            fout.write(f"ECE (10-bin)           : {ece_glob:.4f}\n\n")
